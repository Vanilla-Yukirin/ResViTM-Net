...
# 添加项目根目录到 sys.path
# Augmentation_analysis.py 在 ResViTM-Net/Augmentation_analysis/ 目录下
# 需要再上一级到达项目根目录
...
def save_model(model, loss, timestamp, epoch, model_path='model_output', end=""):
    """保存模型"""
    model_dir = os.path.join(model_path, 'Augmentation_analysis', 'ResViTM')
...
def plot_history(train_losses, val_losses, train_accs, val_accs, timestamp):
    ...
    plt.tight_layout()
    model_dir = os.path.join('model_output', 'Augmentation_analysis', 'ResViTM')
    os.makedirs(model_dir, exist_ok=True)
...
def train_ResViTM(data_lists, num_epochs, batch_size, learning_rate, resume_training, model_path):
    ...
    # 输入增强策略组合 (格式: "abc" 其中 a/b/c 为 0 或 1)
    # a: rotation, b: cropping, c: brightness
    # 例如: "010" 表示只使用 cropping
    augmentation_strategy = input("请输入增强策略 (格式: 'XYZ', 其中X/Y/Z为0或1表示Rotation/Cropping/Brightness): ")
    
    # 验证输入格式
    if len(augmentation_strategy) != 3 or not all(c in '01' for c in augmentation_strategy):
        print("输入格式错误! 使用默认策略 '111'")
        augmentation_strategy = "111"
    
    # 解析增强策略
    use_rotation = augmentation_strategy[0] == '1'
    use_cropping = augmentation_strategy[1] == '1'
    use_brightness = augmentation_strategy[2] == '1'
    
    print(f"\n数据增强策略: Rotation={use_rotation}, Cropping={use_cropping}, Brightness={use_brightness}\n")
    ...
    # 保存报告
    report_dir = os.path.join('report', 'Augmentation_analysis', 'ResViTM')
    os.makedirs(report_dir, exist_ok=True)
    report_path = os.path.join(report_dir, f'{augmentation_strategy}_ResViTM_report-{timestamp}.txt')
