'''
本代码将测试ResViTM_FocalLoss模型。输入是1024*1024的图片，输出是0或1，0表示正常，1表示异常。
使用read_data_meta.py中的main函数读取数据集，并使用ResViTM_FocalLoss模型进行训练。
模型保存在model_output文件夹下
文件名形如
ResViTM_FocalLoss_FocalLoss-20250409010540-{loss:.4f}.pth
'''
...
def save_model(model, loss, timestamp, epoch, model_path='model_output', end=""):
    """保存模型"""
    model_dir = os.path.join(model_path, 'Adjust_Loss', 'FocalLoss')
    os.makedirs(model_dir, exist_ok=True)
    loss_str = f"{loss:.4f}"
    model_name = f'ResViTM_FocalLoss-{timestamp}-{epoch}-{loss_str}{end}.pth'
...
def plot_history(train_losses, val_losses, train_accs, val_accs, timestamp):
    ...
    plt.tight_layout()
    model_dir = os.path.join('model_output', 'Adjust_Loss', 'FocalLoss')
    os.makedirs(model_dir, exist_ok=True)
    plt.savefig(os.path.join(model_dir, f'ResViTM_FocalLoss-{timestamp}.png'))
    # plt.show()
...
def train_ResViTM_FocalLoss(data_lists, num_epochs, batch_size, learning_rate, resume_training, model_path):
    report=""
    """训练ResViTM_FocalLoss模型"""
    # 合并数据列表
    all_data = []
    ...
    if resume_training and model_path:
        ...
        # 从文件名中提取epoch信息
        # 格式: ResViTM_FocalLoss-timestamp-epoch-loss.pth
        try:
        ...
        with torch.no_grad():
            for i in val_pbar:
                ...
                for pred, true in zip(predicted.cpu(), Y_batch.cpu()):
                    confusion_matrix[pred.item(), true.item()] += 1
                    if true.item() == 1:
                        if pred.item() == 1:
                            tp += 1
                        else:
    ...
    # 保存报告
    report_dir = os.path.join('report', 'Adjust_Loss', 'FocalLoss')
    os.makedirs(report_dir, exist_ok=True)
    report_path = os.path.join(report_dir, f'ResViTM_FocalLoss_report-{timestamp}.txt')
...
def select_model():
    ...
    model_files = sorted([f for f in os.listdir(model_dir) if f.endswith('.pth') and f.startswith('ResViTM_FocalLoss')])
    if not model_files:
...
def main():
    ...
    # 训练模型
    best_model, best_val_loss = train_ResViTM_FocalLoss(
