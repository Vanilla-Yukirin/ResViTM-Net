...
def save_model(model, loss, timestamp, epoch, model_path='model_output', end=""):
    """保存模型"""
    model_dir = os.path.join(model_path, 'Adjust_Loss', 'BCELoss')
...
def plot_history(train_losses, val_losses, train_accs, val_accs, timestamp):
    ...
    plt.tight_layout()
    model_dir = os.path.join('model_output', 'Adjust_Loss', 'BCELoss')
    os.makedirs(model_dir, exist_ok=True)
    plt.savefig(os.path.join(model_dir, f'ResViTM_BCELoss-{timestamp}.png'))
    # plt.show()
...
def train_ResViTM_BCELoss(data_lists, num_epochs, batch_size, learning_rate, resume_training, model_path):
    ...
    # 保存报告
    report_dir = os.path.join('report', 'Adjust_Loss', 'BCELoss')
    os.makedirs(report_dir, exist_ok=True)
    report_path = os.path.join(report_dir, f'ResViTM_BCELoss_report-{timestamp}.txt')
