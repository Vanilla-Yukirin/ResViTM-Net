'''
本代码将测试ResViTM_MSELoss模型。输入是1024*1024的图片，输出是0或1，0表示正常，1表示异常。
使用read_data_meta.py中的main函数读取数据集，并使用ResViTM_MSELoss模型进行训练。
模型保存在model_output文件夹下
文件名形如
ResViTM_MSELoss-20250409010540-{loss:.4f}.pth
'''
...
def train_ResViTM_MSELoss(data_lists, num_epochs, batch_size, learning_rate, resume_training, model_path):
    report=""
    """训练ResViTM_MSELoss模型"""
    # 合并数据列表
    all_data = []
    ...
    if resume_training and model_path:
        ...
        # 从文件名中提取epoch信息
        # 格式: ResViTM_MSELoss-timestamp-epoch-loss.pth
        try:
...
def select_model():
    ...
    model_files = sorted([f for f in os.listdir(model_dir) if f.endswith('.pth') and f.startswith('ResViTM_MSELoss')])
    if not model_files:
...
def main():
    ...
    # 训练模型
    best_model, best_val_loss = train_ResViTM_MSELoss(
