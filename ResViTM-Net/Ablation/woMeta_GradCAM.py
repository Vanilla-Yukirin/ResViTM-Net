'''
本代码将测试woMeta模型。输入是1024*1024的图片，输出是0或1，0表示正常，1表示异常。
使用read_data_meta.py中的main函数读取数据集，并使用woMeta模型进行GradCAM可视化。
模型保存在model_output文件夹下
文件名形如
woMeta-20250409010540-{loss:.4f}.pth
...
def save_model(model, loss, timestamp, epoch, model_path='model_output', end=""):
    ...
    loss_str = f"{loss:.4f}"
    model_name = f'woMeta-{timestamp}-{epoch}-{loss_str}{end}.pth'
...
def plot_history(train_losses, val_losses, train_accs, val_accs, timestamp):
    ...
    plt.savefig(f'model_output/woMeta-{timestamp}.png')
    # plt.show()
...
def train_woMeta(data_lists, num_epochs, batch_size, learning_rate, resume_training, model_path):
    report=""
    """训练woMeta模型"""
    # 合并数据列表
    all_data = []
    ...
    if resume_training and model_path:
        ...
        # 从文件名中提取epoch信息
        # 格式: woMeta-timestamp-epoch-loss.pth
        try:
...
def select_model():
    ...
    model_files = sorted([f for f in os.listdir(model_dir) if f.endswith('.pth') and f.startswith('woMeta')])
    if not model_files:
...
def generate_gradcam(model, img, meta, target_class=1, save_path="GradCAM_output/woMeta/sample0.png"):
    """
    生成GradCAM热力图 - woMeta模型版本
    Args:
        model: 训练好的woMeta模型
...
def main():
    ...
    if mode == 1:
        ...
        # 训练模型
        best_model, best_val_loss = train_woMeta(
        ...
        # 创建输出目录
        timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
        output_dir = f"GradCAM_output/woMeta_{timestamp}"
        ...
        for choose_dataset_id, dataset in enumerate(data_lists):
            for choose_data_id, sample in enumerate(dataset):
                ...
                save_path = os.path.join(output_dir, 
                    f"GradCAM_woMeta_data{choose_dataset_id}_{choose_data_id}_{sample['file_name']}_meta{meta_str}_label{sample['positive']}.png")
        ...
        print(f"woMeta模型GradCAM可视化完成！共处理 {cnt} 个样本")
