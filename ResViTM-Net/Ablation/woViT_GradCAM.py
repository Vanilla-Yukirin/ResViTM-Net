'''
本代码将测试woViT模型。输入是1024*1024的图片，输出是0或1，0表示正常，1表示异常。
使用read_data_meta.py中的main函数读取数据集，并使用woViT模型进行GradCAM可视化。
模型保存在model_output文件夹下
文件名形如
woViT-20250409010540-{loss:.4f}.pth

使用元数据
新增：GradCAM可视化功能 - 在CNN部分生成热力图
'''
...
class woViT(nn.Module):
    def __init__(self, img_size=1024, patch_size=16, in_channels=1, num_classes=2,
        ...
        # 用4层最大池化和展平，代替ViT，输出长度为embed_dim的矩阵
        self.pool_size = int(np.sqrt(self.img_size_after_cnn**2 // embed_dim))
        self.max_pool = nn.MaxPool2d(kernel_size=self.pool_size, stride=self.pool_size)
        
        self.flattened_size = (self.img_size_after_cnn // self.pool_size)**2 * in_channels
        self.fc = nn.Linear(self.flattened_size, embed_dim) if self.flattened_size != embed_dim else nn.Identity()
    ...
    def forward(self, x, meta_data):
        ...
        # MaxPool代替ViT
        x = self.max_pool(x)  # [B, in_channels, H/pool_size, W/pool_size]
        x = x.flatten(1)  # [B, in_channels * (H/pool_size)^2]
        ...
        # 分类
        # x = self.head(cls_features)  # [B, num_classes]
        x = self.head(cls_features).squeeze(1)  # 展平输出 [B, 1] -> [B]

        # return x
        return torch.sigmoid(x)


def save_model(model, loss, timestamp, epoch, model_path=os.path.join('model_output', 'Ablation', 'woViT'), end=""):
    """保存模型"""
    os.makedirs(model_path, exist_ok=True)
    loss_str = f"{loss:.4f}"
    model_name = f'woViT-{timestamp}-{epoch}-{loss_str}{end}.pth'
...
def plot_history(train_losses, val_losses, train_accs, val_accs, timestamp):
    ...
    plt.tight_layout()
    save_dir = os.path.join('model_output', 'Ablation', 'woViT')
    os.makedirs(save_dir, exist_ok=True)
    plt.savefig(os.path.join(save_dir, f'woViT-{timestamp}.png'))
    # plt.show()
...
def train_woViT(data_lists, num_epochs, batch_size, learning_rate, resume_training, model_path):
    report=""
    """训练woViT模型"""
    # 合并数据列表
    all_data = []
    ...
    if resume_training and model_path:
        ...
        # 从文件名中提取epoch信息
        # 格式: woViT-timestamp-epoch-loss.pth
        try:
    ...
    with torch.no_grad():
        for i in range(0, len(val_data), batch_size):
            ...
            outputs = best_model(X_batch, meta_batch)
            probabilities = torch.softmax(outputs, dim=1)
            
            all_predictions.extend(probabilities[:, 1].cpu().numpy())  # 获取正类的概率
        ...
        # 保存ROC曲线图
        plt.savefig(f'plots/ROC_curve_{timestamp}.png')
        plt.close()
    ...
    all_labels=[]
    with torch.no_grad():
        for i in range(0, 24, batch_size): # 展示几个batch
            batch_data = val_data[i:i + batch_size]
            print(batch_data)
            # 准备批次数据
            X_batch = torch.stack([torch.from_numpy(d["img"]).unsqueeze(0).float() / 255.0 for d in batch_data]).to(device)
    ...
    # 保存报告
    report_dir = os.path.join('report', 'Ablation', 'woViT')
    os.makedirs(report_dir, exist_ok=True)
    report_path = os.path.join(report_dir, f'woViT_report-{timestamp}.txt')
...
def select_model():
    """选择要使用的模型文件"""
    model_dir = os.path.join('model_output', 'Ablation', 'woViT')
    ...
    model_files = sorted([f for f in os.listdir(model_dir) if f.endswith('.pth') and f.startswith('woViT')])
    if not model_files:
...
def generate_gradcam(model, img, meta, target_class=1, save_path="GradCAM_output/woViT/sample0.png"):
    """
    生成GradCAM热力图 - woViT模型版本
    Args:
        model: 训练好的woViT模型
        img: 输入图像 numpy array [H, W]
        meta: 元数据 list [gender0, gender1, age0, age1, age2]
        target_class: 目标类别 (1表示异常)
        save_path: 保存路径
    """
    import torch.nn.functional as F
    import cv2
    ...
    # 准备输入数据
    input_img = torch.from_numpy(img).unsqueeze(0).unsqueeze(0).float().to(device) / 255.0
    ...
    def forward_hook(module, input, output):
        activations['cnn_out'] = output
    
    def backward_hook(module, grad_in, grad_out):
        activations['cnn_grad'] = grad_out[0]
    
    # 注册hook到CNN的final_conv层，这是特征提取的最后一层
    handle_f = model.cnn.final_conv.register_forward_hook(forward_hook)
    handle_b = model.cnn.final_conv.register_full_backward_hook(backward_hook)
    
    try:
        # 前向传播
        output = model(input_img, meta_tensor)
        
        # 对于sigmoid输出，直接使用输出作为score
        score = output[0] if target_class == 1 else (1 - output[0])
        ...
        # 生成GradCAM
        gradcam = torch.sum(weights * cnn_out, dim=1, keepdim=True)  # [1, 1, H/16, W/16]
        gradcam = F.relu(gradcam)  # 只保留正值
        
        # 上采样到原图大小
        gradcam = F.interpolate(gradcam, size=(1024, 1024), mode='bilinear', align_corners=False)
        gradcam = gradcam.squeeze().cpu().detach().numpy()  # [1024, 1024]
        ...
        if np.any(mask):
            ...
            gradcam_masked = np.log(gradcam_masked + 1e-6)
            gradcam_log = np.zeros_like(gradcam)
            gradcam_log[mask] = gradcam_masked
            gradcam = gradcam_log
            
            # 归一化到[0,1]
            gradcam_masked = gradcam[mask]
            gradcam_min = gradcam_masked.min()
            gradcam_max = gradcam_masked.max()
            gradcam = (gradcam - gradcam_min) / (gradcam_max - gradcam_min + 1e-8)
            gradcam[~mask] = 0  # 背景区域设为0
        else:
            # 如果没有mask，直接归一化
            gradcam -= gradcam.min()
...
def main():
    ...
    if mode == 1:
        ...
        # 训练模型
        best_model, best_val_loss = train_woViT(
        ...
        # 创建输出目录
        timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
        output_dir = f"GradCAM_output/woViT_{timestamp}"
        ...
        for choose_dataset_id, dataset in enumerate(data_lists):
            for choose_data_id, sample in enumerate(dataset):
                ...
                save_path = os.path.join(output_dir, 
                    f"GradCAM_woViT_data{choose_dataset_id}_{choose_data_id}_{sample['file_name']}_meta{meta_str}_label{sample['positive']}.png")
        ...
        print(f"woViT模型GradCAM可视化完成！共处理 {cnt} 个样本")
