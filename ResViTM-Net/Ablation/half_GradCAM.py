'''
本代码将测试half模型。输入是1024*1024的图片，输出是0或1，0表示正常，1表示异常。
使用read_data_meta.py中的main函数读取数据集，并使用half模型进行GradCAM可视化。
模型保存在model_output文件夹下
文件名形如
half-20250409010540-{loss:.4f}.pth
...
class HalfResNetCNN(nn.Module):
    def __init__(self):
        super().__init__()
        # 1. 加载预训练的ResNet18模型
        resnet18 = models.resnet18(weights=models.ResNet18_Weights.IMAGENET1K_V1)

        # 2. 截取到 layer2 结束 (原版是到layer3)
        # ResNet18的子模块: [conv1, bn1, relu, maxpool, layer1, layer2, layer3, layer4, avgpool, fc]
        # 我们取到-4之前，即到 layer2
        feature_extractor_layers = list(resnet18.children())[:-4]
        self.features_pre = nn.Sequential(*feature_extractor_layers)
        
        # 3. 手动添加一个MaxPool层
        # 到layer2为止，下采样倍数是 2(conv1) * 2(maxpool) * 2(layer2) = 8倍
        # 我们需要16倍，所以再加一个2倍的MaxPool
        self.features_post = nn.MaxPool2d(kernel_size=2, stride=2)

        # 4. 修改最后的1x1卷积
        # ResNet18的layer2输出通道是128 (layer3是256)
        # 所以这里的输入通道数要从256改为128
...
class half(nn.Module):
    def __init__(self, img_size=1024, patch_size=16, in_channels=1, num_classes=2,
        ...
        self.head = nn.Sequential(
            # nn.Linear(embed_dim, embed_dim), # halfhalf
            # nn.ReLU(),
            # nn.Dropout(0.2),
        ...
        self.meta_net=nn.Sequential(
            ...
            # nn.ReLU(),
            # nn.Dropout(0.2),
            nn.Linear(32, embed_dim) # 输出维度保持与ViT一致
        )
...
def save_model(model, loss, timestamp, epoch, model_path='model_output', end=""):
    ...
    loss_str = f"{loss:.4f}"
    model_name = f'half-{timestamp}-{epoch}-{loss_str}{end}.pth'
...
def plot_history(train_losses, val_losses, train_accs, val_accs, timestamp):
    ...
    plt.savefig(f'model_output/half-{timestamp}.png')
    # plt.show()
...
def train_half(data_lists, num_epochs, batch_size, learning_rate, resume_training, model_path):
    report=""
    """训练half模型"""
    # 合并数据列表
    all_data = []
    ...
    if resume_training and model_path:
        ...
        # 从文件名中提取epoch信息
        # 格式: half-timestamp-epoch-loss.pth
        try:
...
def select_model():
    ...
    model_files = sorted([f for f in os.listdir(model_dir) if f.endswith('.pth') and f.startswith('half')])
    if not model_files:
...
def main():
    ...
    if mode == 1:
        ...
        # 训练模型
        best_model, best_val_loss = train_half(
        ...
        # 创建输出目录
        timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
        output_dir = os.path.join('GradCAM_output', f"half_{timestamp}")
            ...
            for choose_dataset_id, dataset in enumerate(data_lists):
                for choose_data_id, sample in enumerate(dataset):
                    ...
                    save_path = os.path.join(output_dir, 
                        f"GradCAM_half_data{choose_dataset_id}_{choose_data_id}_{sample['file_name']}_meta{meta_str}_label{sample['positive']}.png")
        ...
        print(f"half模型GradCAM可视化完成！共处理 {cnt} 个样本")
