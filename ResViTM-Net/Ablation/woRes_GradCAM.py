'''
本代码将测试woRes模型。输入是1024*1024的图片，输出是0或1，0表示正常，1表示异常。
使用read_data_meta.py中的main函数读取数据集，并使用woRes模型进行GradCAM可视化。
模型保存在model_output文件夹下
文件名形如
woRes-20250409010540-{loss:.4f}.pth

使用元数据
新增：GradCAM可视化功能 - 在PatchEmbedding层生成热力图
'''
...
def save_model(model, loss, timestamp, epoch, model_path='model_output', end=""):
    ...
    loss_str = f"{loss:.4f}"
    model_name = f'woRes-{timestamp}-{epoch}-{loss_str}{end}.pth'
...
def plot_history(train_losses, val_losses, train_accs, val_accs, timestamp):
    ...
    plt.savefig(f'model_output/woRes-{timestamp}.png')
    # plt.show()
...
def train_woRes(data_lists, num_epochs, batch_size, learning_rate, resume_training, model_path):
    report=""
    """训练woRes模型"""
    # 合并数据列表
    all_data = []
    ...
    train_augmenter = ReproducibleAugmenter(
        ...
        num_augmentations_per_image=0, # 每个样本额外生成2个
        use_rotation=True,
        use_cropping=True,
        use_brightness=True
    )
    ...
    if resume_training and model_path:
        ...
        # 从文件名中提取epoch信息
        # 格式: woRes-timestamp-epoch-loss.pth
        try:
    ...
    # 保存报告
    report_path = os.path.join('report', f'woRes_report-{timestamp}.txt')
...
def select_model():
    ...
    model_files = sorted([f for f in os.listdir(model_dir) if f.endswith('.pth') and f.startswith('woRes')])
    if not model_files:
...
def generate_gradcam(model, img, meta, target_class=1, save_path=os.path.join("GradCAM_output", "Ablation", "woRes", "sample0.png")):
    """
    生成GradCAM热力图 - woRes模型版本
    Args:
        model: 训练好的woRes模型
    ...
    def forward_hook(module, input, output):
        activations['patch_out'] = output
    
    def backward_hook(module, grad_in, grad_out):
        activations['patch_grad'] = grad_out[0]
    
    # 注册hook到PatchEmbedding的proj层，这是特征提取的唯一层
    handle_f = model.patch_embed.proj.register_forward_hook(forward_hook)
    handle_b = model.patch_embed.proj.register_full_backward_hook(backward_hook)
    ...
    try:
        ...
        # 获取激活值和梯度
        patch_out = activations['patch_out']  # [1, embed_dim, H/patch_size, W/patch_size]
        patch_grad = activations['patch_grad']  # [1, embed_dim, H/patch_size, W/patch_size]
        
        print(f"梯度值范围: {patch_grad.min().item():.6f} - {patch_grad.max().item():.6f}")
        
        # 计算权重 (对空间维度求平均)
        weights = torch.mean(patch_grad, dim=(2, 3), keepdim=True)  # [1, embed_dim, 1, 1]
        
        # 生成GradCAM
        gradcam = torch.sum(weights * patch_out, dim=1, keepdim=True)  # [1, 1, H/patch_size, W/patch_size]
...
def main():
    ...
    if mode == 1:
        ...
        # 训练模型
        best_model, best_val_loss = train_woRes(
        ...
        # 创建输出目录
        timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
        output_dir = os.path.join('GradCAM_output', 'Ablation', 'woRes', f"woRes_{timestamp}")
        ...
        for choose_dataset_id, dataset in enumerate(data_lists):
            for choose_data_id, sample in enumerate(dataset):
                ...
                save_path = os.path.join(output_dir, 
                    f"GradCAM_woRes_data{choose_dataset_id}_{choose_data_id}_{sample['file_name']}_meta{meta_str}_label{sample['positive']}.png")
        ...
        print(f"woRes模型GradCAM可视化完成！共处理 {cnt} 个样本")
