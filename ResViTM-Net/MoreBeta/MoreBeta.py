...
输入 1024*1024的图片
输出 0或1，0表示正常，1表示异常。
使用read_data_meta.py中的main函数读取数据集
使用ResViTM-Net模型进行训练。
模型保存在model_output文件夹下
文件名形如 ResViTM-20250409010540-{loss:.4f}.pth
'''
...
def save_model(model, loss, timestamp, epoch, beta_str, model_path='model_output', end=""):
    """保存模型"""
    model_dir = os.path.join(model_path, 'MoreBeta', beta_str)
...
def plot_history(train_losses, val_losses, train_accs, val_accs, timestamp, beta_str):
    """绘制训练历史"""
...
def train_ResViTM(data_lists, num_epochs, batch_size, learning_rate, resume_training, model_path, beta_value, beta_str):
    report=""
    """训练ResViTM模型"""
    # 合并数据列表
    all_data = []
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        # 保存当前模型
        timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
        save_model(model, avg_val_loss, timestamp, epoch+1, beta_str)
    ...
    timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
    plot_history(train_losses, val_losses, train_accs, val_accs, timestamp, beta_str)
    
    # 保存最佳模型
    if best_model is not None:
        
        best_model_path = save_model(best_model, best_val_loss, timestamp, best_epoch_id, beta_str, end="-best")
        print(f"最佳模型已保存: {best_model_path}, Val Loss: {best_val_loss:.4f}")
        report += f"最佳模型已保存: {best_model_path}, Val Loss: {best_val_loss:.4f}\n"
    
    
    # 保存报告
    report_dir = os.path.join('report', 'MoreBeta', beta_str)
...
def main():
    # 命令行参数解析
    parser = argparse.ArgumentParser(description='MoreBeta ResViTM 模型训练')
    parser.add_argument('-b', '--beta', type=float, required=True, 
                       help='SmoothL1Loss的beta参数，范围0.0-1.0，保留一位小数')
    
    try:
        args = parser.parse_args()
    except SystemExit:
        print("错误: 必须使用 -b 参数指定beta值")
        print("用法: python MoreBeta.py -b 0.x")
        sys.exit(1)
    
    # 验证beta值
    beta_value = args.beta
    if not (0.0 <= beta_value <= 1.0):
        print(f"错误: beta值必须在0.0-1.0之间，你输入的是 {beta_value}")
        sys.exit(1)
    
    # 验证是否是一位小数
    beta_str_temp = f"{beta_value:.1f}"
    if float(beta_str_temp) != beta_value:
        print(f"错误: beta值必须保留一位小数，你输入的是 {beta_value}")
        sys.exit(1)
    
    beta_str = beta_str_temp
    
    print(f"使用的beta值: {beta_value}, 路径标识符: {beta_str}")
