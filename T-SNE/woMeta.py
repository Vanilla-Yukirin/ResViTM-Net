...
使用read_data_meta.py中的main函数读取数据集，并使用woMeta模型进行训练。
...
class woMeta(nn.Module):
    ...
    def extract_features(self, x, meta_data):
        ...
        Args:
            ...
            meta_data: 元数据输入 [B, 5]
        Returns:
            features: 最后一层特征 [B, embed_dim]
        """
        return self.forward(x, meta_data, return_features=True)


def save_model(model, loss, timestamp, epoch, model_path=os.path.join('model_output', 'Ablation', 'woMeta'), end=""):
    """保存模型"""
...
def plot_history(train_losses, val_losses, train_accs, val_accs, timestamp):
    ...
    plt.tight_layout()
    save_dir = os.path.join('model_output', 'Ablation', 'woMeta')
    os.makedirs(save_dir, exist_ok=True)
    plt.savefig(os.path.join(save_dir, f'woMeta-{timestamp}.png'))
    # plt.show()
...
def select_model():
    """选择要使用的模型文件"""
    model_dir = os.path.join('model_output', 'Ablation', 'woMeta')
