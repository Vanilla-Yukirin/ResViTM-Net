'''
本代码将测试RegnetX_400MF模型。输入是1024*1024的图片，输出是0或1，0表示正常，1表示异常。
使用read_data_meta.py中的main函数读取数据集，并使用RegnetX_400MF模型进行训练。
模型保存在model_output文件夹下
文件名形如
RegnetX_400MF-20250409010540-{loss:.4f}.pth
...
class RegnetX_400MF(nn.Module):
    def __init__(self, num_classes=2):
        super().__init__()
        # 加载预训练的 RegNet
        regnet_model = models.regnet_x_400mf(weights=RegNet_X_400MF_Weights.IMAGENET1K_V1)
        
        # 分离特征提取和分类器
        self.stem = regnet_model.stem
        self.trunk_output = regnet_model.trunk_output
        self.avgpool = regnet_model.avgpool
        
        # 获取输入维度并创建新的分类器
        num_ftrs = regnet_model.fc.in_features # 获取输入维度
...
def save_model(model, loss, timestamp, epoch, model_path='model_output', end=""):
    """保存模型"""
    model_dir = os.path.join(model_path, 'Model_Compare', 'RegnetX_400MF')
    os.makedirs(model_dir, exist_ok=True)
    loss_str = f"{loss:.4f}"
    model_name = f'RegnetX_400MF-{timestamp}-{epoch}-{loss_str}{end}.pth'
...
def plot_history(train_losses, val_losses, train_accs, val_accs, timestamp):
    ...
    os.makedirs(model_dir, exist_ok=True)
    plt.savefig(os.path.join(model_dir, f'RegnetX_400MF-{timestamp}.png'))
    # plt.show()
...
def train_cnn_vit_meta(data_lists, num_epochs, batch_size, learning_rate, resume_training, model_path):
    ...
    if resume_training and model_path:
        ...
        try:
            ...
            for k, v in state.items():
                kl = k.lower()
                if kl.startswith('model.fc') or '.fc.' in kl or kl.startswith('classifier'):
                    dropped.append(k)
                    continue
                filtered_state[k] = v
    ...
    # 保存报告
    # report_dir = os.path.join('report', 'Model_Compare', 'RegnetX_400MF')
    # os.makedirs(report_dir, exist_ok=True)
    # report_path = os.path.join(report_dir, f'RegnetX_400MF_report-{timestamp}.txt')
