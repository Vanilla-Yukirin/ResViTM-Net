'''
本代码将测试ResNet34模型。输入是1024*1024的图片，输出是0或1，0表示正常，1表示异常。
使用read_data_meta.py中的main函数读取数据集，并使用ResNet34模型进行训练。
模型保存在model_output文件夹下
文件名形如
ResNet34-20250409010540-{loss:.4f}.pth
...
def save_model(model, loss, timestamp, epoch, model_path='model_output', end=""):
    """保存模型"""
    model_dir = os.path.join(model_path, 'Model_Compare', 'Resnet34')
    os.makedirs(model_dir, exist_ok=True)
    loss_str = f"{loss:.4f}"
    model_name = f'Resnet34-{timestamp}-{epoch}-{loss_str}{end}.pth'
...
def plot_history(train_losses, val_losses, train_accs, val_accs, timestamp):
    ...
    plt.tight_layout()
    model_dir = os.path.join('model_output', 'Model_Compare', 'Resnet34')
    os.makedirs(model_dir, exist_ok=True)
    plt.savefig(os.path.join(model_dir, f'Resnet34-{timestamp}.png'))
    # plt.show()
...
def train_cnn_vit_meta(data_lists, num_epochs, batch_size, learning_rate, resume_training, model_path):
    ...
    if resume_training and model_path:
        ...
        try:
            ...
            for k, v in state.items():
                kl = k.lower()
                if kl.startswith('resnet.fc') or '.fc.' in kl or kl.startswith('classifier') or kl.startswith('feature_extractor'):
                    dropped.append(k)
                    continue
                filtered_state[k] = v
    ...
    # 保存报告
    # report_dir = os.path.join('report', 'Model_Compare', 'Resnet34')
    # os.makedirs(report_dir, exist_ok=True)
    # report_path = os.path.join(report_dir, f'Resnet34_report-{timestamp}.txt')
...
def select_model():
    ...
    model_files = sorted([f for f in os.listdir(model_dir) if f.endswith('.pth') and f.startswith('Resnet34')])
    if not model_files:
