'''
本代码将测试ResNet18模型。输入是1024*1024的图片，输出是0或1，0表示正常，1表示异常。
使用read_data_meta.py中的main函数读取数据集，并使用ResNet18模型进行训练。
模型保存在model_output文件夹下
文件名形如
ResNet18-20250409010540-{loss:.4f}.pth
...
class ResNet18(nn.Module):
    def __init__(self, num_classes=2):
        ...
        # 替换ResNet的fc层
        self.ResNet.fc = nn.Identity()  # 移除原始分类器
        
    def forward(self, x, return_features=False):
        # 提取ResNet特征
        backbone_features = self.ResNet(x)  # [B, 512]
        
        # 通过特征提取器
        features = self.feature_extractor(backbone_features)  # [B, 512]
        ...
        # 分类
        x = self.classifier(features).squeeze(1)
...
def save_model(model, loss, timestamp, epoch, model_path='model_output', end=""):
    """保存模型"""
    model_dir = os.path.join(model_path, 'Model_Compare', 'Resnet18')
    os.makedirs(model_dir, exist_ok=True)
    loss_str = f"{loss:.4f}"
    model_name = f'Resnet18-{timestamp}-{epoch}-{loss_str}{end}.pth'
...
def plot_history(train_losses, val_losses, train_accs, val_accs, timestamp):
    ...
    plt.tight_layout()
    model_dir = os.path.join('model_output', 'Model_Compare', 'Resnet18')
    os.makedirs(model_dir, exist_ok=True)
    plt.savefig(os.path.join(model_dir, f'Resnet18-{timestamp}.png'))
    # plt.show()
...
def train_cnn_vit_meta(data_lists, num_epochs, batch_size, learning_rate, resume_training, model_path):
    ...
    if resume_training and model_path:
        ...
        # 兼容旧checkpoint：丢弃分类头等不匹配参数，加载其余权重
        try:
            ...
            for k, v in state.items():
                kl = k.lower()
                if ('.fc.' in kl) or kl.startswith('resnet.fc') or kl.startswith('resnet18.fc') or kl.startswith('resnet_18.fc') or kl.startswith('resnet.fc') or kl.startswith('resnet') and ('fc.' in kl) or kl.startswith('classifier') or kl.startswith('feature_extractor'):
                    dropped.append(k)
                    continue
                filtered_state[k] = v
    ...
    # 保存报告
    # report_dir = os.path.join('report', 'Model_Compare', 'Resnet18')
    # os.makedirs(report_dir, exist_ok=True)
    # report_path = os.path.join(report_dir, f'Resnet18_report-{timestamp}.txt')
...
def select_model():
    ...
    model_files = sorted([f for f in os.listdir(model_dir) if f.endswith('.pth') and f.startswith('Resnet18')])
    if not model_files:
