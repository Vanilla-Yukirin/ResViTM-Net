'''
本代码将测试MobilenetV2模型。输入是1024*1024的图片，输出是0或1，0表示正常，1表示异常。
使用read_data_meta.py中的main函数读取数据集，并使用MobilenetV2模型进行训练。
模型保存在model_output文件夹下
文件名形如
MobilenetV2-20250409010540-{loss:.4f}.pth
...
class MobilenetV2(nn.Module):
    def __init__(self, num_classes=2):
        super().__init__()
        # 加载预训练的 MobileNetV2
        mobilenet_model = models.mobilenet_v2(weights=MobileNet_V2_Weights.IMAGENET1K_V1)
        ...
        # 获取原始分类器的输入维度
        num_ftrs = mobilenet_model.classifier[1].in_features # 获取输入维度
        self.classifier = nn.Linear(num_ftrs, num_classes) # 替换
...
def save_model(model, loss, timestamp, epoch, model_path='model_output', end=""):
    """保存模型"""
    model_dir = os.path.join(model_path, 'Model_Compare', 'MobilenetV2')
    os.makedirs(model_dir, exist_ok=True)
    loss_str = f"{loss:.4f}"
    model_name = f'MobilenetV2-{timestamp}-{epoch}-{loss_str}{end}.pth'
...
def plot_history(train_losses, val_losses, train_accs, val_accs, timestamp):
    ...
    plt.tight_layout()
    model_dir = os.path.join('model_output', 'Model_Compare', 'MobilenetV2')
    os.makedirs(model_dir, exist_ok=True)
    plt.savefig(os.path.join(model_dir, f'MobilenetV2-{timestamp}.png'))
    # plt.show()
...
def train_cnn_vit_meta(data_lists, num_epochs, batch_size, learning_rate, resume_training, model_path):
    ...
    if resume_training and model_path:
        ...
        # 兼容旧checkpoint：MobileNet的classifier[1]可能命名不同
        try:
            ...
            for k, v in state.items():
                kl = k.lower()
                if kl.startswith('model.classifier') or '.classifier.' in kl:
                    dropped.append(k)
                    continue
                filtered_state[k] = v
    ...
    # 保存报告
    # report_dir = os.path.join('report', 'Model_Compare', 'MobilenetV2')
    # os.makedirs(report_dir, exist_ok=True)
    # report_path = os.path.join(report_dir, f'MobilenetV2_report-{timestamp}.txt')
...
def select_model():
    ...
    model_files = sorted([f for f in os.listdir(model_dir) if f.endswith('.pth') and f.startswith('MobilenetV2')])
    if not model_files:
