...
使用read_data_meta.py中的main函数读取数据集，并使用woRes模型进行训练。
...
def save_model(model, loss, timestamp, epoch, model_path=os.path.join('model_output', 'Ablation', 'woRes'), end=""):
    """保存模型"""
...
def plot_history(train_losses, val_losses, train_accs, val_accs, timestamp):
    ...
    plt.tight_layout()
    save_dir = os.path.join('model_output', 'Ablation', 'woRes')
    os.makedirs(save_dir, exist_ok=True)
    plt.savefig(os.path.join(save_dir, f'woRes-{timestamp}.png'))
    # plt.show()
...
def select_model():
    """选择要使用的模型文件"""
    model_dir = os.path.join('model_output', 'Ablation', 'woRes')
