'''
本代码将测试ViT16_224模型。输入是1024*1024的图片，输出是0或1，0表示正常，1表示异常。
使用read_data_meta.py中的main函数读取数据集，并使用ViT16_224模型进行训练。
模型保存在model_output文件夹下
文件名形如
ViT16_224-20250409010540-{loss:.4f}.pth
...
class ViT16_224(nn.Module):
    def __init__(self, num_classes=2):
        super().__init__()
        # 使用 timm.create_model 来创建 ViT 模型
        self.vit = timm.create_model(
            'vit_base_patch16_224', # 模型名称
            pretrained=True,        # 使用预训练权重
            num_classes=num_classes,# 直接设置我们需要的输出类别数
            in_chans=1,             # 关键：告诉模型输入是1通道
            # timm 库会自动处理位置编码的尺寸问题，通常不需要额外参数。
            img_size=1024           # 明确告诉模型新的输入尺寸是1024x1024
        )

    def forward(self, x, return_features=False):
        # 如果需要返回特征，我们需要通过vit的forward_features方法
        if return_features:
            # 通过ViT的特征提取部分，获取最后一层transformer的输出
            features = self.vit.forward_features(x)
            # ViT通常返回[CLS] token的特征作为全局特征
...
def train_cnn_vit_meta(data_lists, num_epochs, batch_size, learning_rate, resume_training, model_path):
    ...
    model = ViT16_224(num_classes=1)

    # 如果继续训练，加载已保存的模型并解析文件名获取epoch信息
    if resume_training and model_path:
        print(f"加载模型: {model_path}")
        # 兼容旧checkpoint：timm ViT的head尺寸可能不同，过滤掉head相关权重
        try:
            ...
            for k, v in state.items():
                kl = k.lower()
                if '.head.' in kl or kl.endswith('head.weight') or kl.endswith('head.bias') or kl.startswith('classifier'):
                    dropped.append(k)
                    continue
                filtered_state[k] = v
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        for i in train_pbar:
            ...
            X_batch_raw = torch.stack([torch.from_numpy(d["img"]).unsqueeze(0).float() / 255.0 for d in batch_data]).to(device)
            # 直接对单通道数据进行归一化
            X_batch = normalizer(X_batch_raw)
    ...
    try:
        ...
        features, labels = extract_features_for_tsne(
            ...
            needs_3_channels=False  # ViT16_224 使用 1 通道输入，且模型 img_size=1024，无需resize
        )
...
def select_model():
    ...
    model_files = sorted([f for f in os.listdir(model_dir) if f.endswith('.pth') and f.startswith('ViT16_224')])
    if not model_files:
