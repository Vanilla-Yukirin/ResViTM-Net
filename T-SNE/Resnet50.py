'''
本代码将测试Resnet50模型。输入是1024*1024的图片，输出是0或1，0表示正常，1表示异常。
使用read_data_meta.py中的main函数读取数据集，并使用Resnet50模型进行训练。
模型保存在model_output文件夹下
文件名形如
Resnet50-20250409010540-{loss:.4f}.pth
...
class ResNet50(nn.Module):
    ...
    def forward(self, x, return_features=False):
        # 提取ResNet特征
        backbone_features = self.resnet(x)  # [B, 2048]
...
def select_model():
    ...
    model_files = sorted([f for f in os.listdir(model_dir) if f.endswith('.pth') and f.startswith('Resnet50')])
    if not model_files:
