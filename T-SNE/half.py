...
使用read_data_meta.py中的main函数读取数据集，并使用half模型进行训练。
...
class half(nn.Module):
    ...
    def forward(self, x, meta_data, return_features=False):
        """
        前向传播
        Args:
            x: 图像输入 [B, C, H, W]
    ...
    def extract_features(self, x, meta_data):
        """
        提取最后一层全连接层的特征
        """
        return self.forward(x, meta_data, return_features=True)


def save_model(model, loss, timestamp, epoch, model_path=os.path.join('model_output', 'Ablation', 'half'), end=""):
    """保存模型"""
...
def plot_history(train_losses, val_losses, train_accs, val_accs, timestamp):
    ...
    plt.tight_layout()
    save_dir = os.path.join('model_output', 'Ablation', 'half')
    os.makedirs(save_dir, exist_ok=True)
    plt.savefig(os.path.join(save_dir, f'half-{timestamp}.png'))
    # plt.show()
...
def train_half(data_lists, num_epochs, batch_size, learning_rate, resume_training, model_path):
    ...
    try:
        from utils.tsne_utils_new import extract_features_for_tsne, generate_tsne_visualization
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        ...
        features, labels = extract_features_for_tsne(
            ...
            needs_3_channels=False
        )
        
        tsne_output_path, features_2d = generate_tsne_visualization(
            features=features,
            labels=labels,
            model_name="half"
        )
        print(f"t-SNE visualization completed: {tsne_output_path}")
    except Exception as e:
        print(f"Error during t-SNE generation: {e}")
        import traceback
        traceback.print_exc()
...
def select_model():
    """选择要使用的模型文件"""
    model_dir = os.path.join('model_output', 'Ablation', 'half')
