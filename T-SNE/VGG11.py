'''
本代码将测试Vgg11模型。输入是1024*1024的图片，输出是0或1，0表示正常，1表示异常。
使用read_data_meta.py中的main函数读取数据集，并使用Vgg11模型进行训练。
模型保存在model_output文件夹下
文件名形如
Vgg11-20250409010540-{loss:.4f}.pth
...
class Vgg11(nn.Module):
    def __init__(self, num_classes=2):
        super().__init__()
        # 使用预训练的 Vgg11
        vgg_model = models.vgg11(weights=VGG11_Weights.IMAGENET1K_V1)
...
def save_model(model, loss, timestamp, epoch, model_path='model_output', end=""):
    """保存模型"""
    model_dir = os.path.join(model_path, 'Model_Compare', 'VGG11')
    os.makedirs(model_dir, exist_ok=True)
    loss_str = f"{loss:.4f}"
    model_name = f'Vgg11-{timestamp}-{epoch}-{loss_str}{end}.pth'
...
def plot_history(train_losses, val_losses, train_accs, val_accs, timestamp):
    ...
    plt.tight_layout()
    model_dir = os.path.join('model_output', 'Model_Compare', 'VGG11')
    os.makedirs(model_dir, exist_ok=True)
    plt.savefig(os.path.join(model_dir, f'Vgg11-{timestamp}.png'))
    # plt.show()
...
def train_cnn_vit_meta(data_lists, num_epochs, batch_size, learning_rate, resume_training, model_path):
        ...
        try:
            ...
            for k, v in state.items():
                ...
                if kl.startswith('classifier') or ('.classifier.' in kl):
                    # 仅保留除最后线性层前的层可能名称变化，干脆全部丢弃classifier权重
                    dropped.append(k)
                    continue
                filtered_state[k] = v
    ...
    # 保存报告
    # report_dir = os.path.join('report', 'Model_Compare', 'VGG11')
    # os.makedirs(report_dir, exist_ok=True)
    # report_path = os.path.join(report_dir, f'Vgg11_report-{timestamp}.txt')
...
def select_model():
    ...
    model_files = sorted([f for f in os.listdir(model_dir) if f.endswith('.pth') and f.startswith('Vgg11')])
    if not model_files:
