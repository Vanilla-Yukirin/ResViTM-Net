'''
本代码将测试Densenet121模型。输入是1024*1024的图片，输出是0或1，0表示正常，1表示异常。
使用read_data_meta.py中的main函数读取数据集，并使用Densenet121模型进行训练。
模型保存在model_output文件夹下
文件名形如
Densenet121-20250409010540-{loss:.4f}.pth
...
class Densenet121(nn.Module):
    def __init__(self, num_classes=2):
        ...
        # 替换掉原来的分类器
        #    DenseNet的最后一个线性层 'classifier'
        #    获取它原本的输入维度
        num_ftrs = densenet_model.classifier.in_features # 对于DenseNet121, 是 1024
...
def save_model(model, loss, timestamp, epoch, model_path='model_output', end=""):
    """保存模型"""
    model_dir = os.path.join(model_path, 'Model_Compare', 'Densenet121')
    os.makedirs(model_dir, exist_ok=True)
    loss_str = f"{loss:.4f}"
    model_name = f'Densenet121-{timestamp}-{epoch}-{loss_str}{end}.pth'
...
def plot_history(train_losses, val_losses, train_accs, val_accs, timestamp):
    ...
    plt.tight_layout()
    model_dir = os.path.join('model_output', 'Model_Compare', 'Densenet121')
    os.makedirs(model_dir, exist_ok=True)
    plt.savefig(os.path.join(model_dir, f'Densenet121-{timestamp}.png'))
    # plt.show()
...
def train_cnn_vit_meta(data_lists, num_epochs, batch_size, learning_rate, resume_training, model_path):
    ...
    if resume_training and model_path:
        ...
        # 兼容旧checkpoint：DenseNet分类器键名不同，过滤head/classifier相关
        try:
    ...
    # 保存报告
    # report_dir = os.path.join('report', 'Model_Compare', 'Densenet121')
    # os.makedirs(report_dir, exist_ok=True)
    # report_path = os.path.join(report_dir, f'Densenet121_report-{timestamp}.txt')
    ...
    try:
        ...
        features, labels = extract_features_for_tsne(
            ...
            n_samples=n_samples,
            device=device,
            model_needs_meta=False,  # DenseNet121不需要元数据
...
def select_model():
    ...
    model_files = sorted([f for f in os.listdir(model_dir) if f.endswith('.pth') and f.startswith('Densenet121')])
    if not model_files:
