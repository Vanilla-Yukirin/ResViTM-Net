'''
本代码将测试ResViTM模型。输入是1024*1024的图片，输出是0或1，0表示正常，1表示异常。
使用read_data_meta.py中的main函数读取数据集，并使用ResViTM模型进行训练。
模型保存在model_output文件夹下
文件名形如
ResViTM-20250409010540-{loss:.4f}.pth
...
def train_ResViTM(data_lists, num_epochs, batch_size, learning_rate, resume_training, model_path):
    ...
    try:
        ...
        features, labels = extract_features_for_tsne(
            ...
            n_samples=n_samples,
            device=device,
            model_needs_meta=True,  # CViTM需要元数据
            needs_3_channels=False  # CViTM使用单通道输入
        )
