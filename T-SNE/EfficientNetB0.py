'''
本代码将测试EfficientnetB0模型。输入是1024*1024的图片，输出是0或1，0表示正常，1表示异常。
使用read_data_meta.py中的main函数读取数据集，并使用EfficientnetB0模型进行训练。
模型保存在model_output文件夹下
文件名形如
EfficientnetB0-20250409010540-{loss:.4f}.pth
...
class EfficientnetB0(nn.Module):
    def __init__(self, num_classes=2):
        super().__init__()
        # 加载预训练的 efficientnet_b0 模型
        effnet_model = models.efficientnet_b0(weights=EfficientNet_B0_Weights.IMAGENET1K_V1)
        
        # 我们将使用它的特征提取部分和自适应池化层
        self.features = effnet_model.features
        self.avgpool = effnet_model.avgpool
        
        # 替换掉原来的分类器中的最后一个线性层
        #    EfficientNet的 'classifier' 是一个 nn.Sequential, 
        #    通常包含一个 Dropout 和一个 nn.Linear。我们需要替换那个 nn.Linear。
        #    它的索引通常是 [1]。 
        #    获取原本的输入维度
        num_ftrs = effnet_model.classifier[1].in_features # 对于 B0, 这是 1280
        #    用一个新的、输出为我们所需类别数的线性层来替换它
        effnet_model.classifier[1] = nn.Linear(num_ftrs, num_classes)
        #    现在整个修改后的 classifier 模块都可以用了
        self.classifier = effnet_model.classifier
...
def save_model(model, loss, timestamp, epoch, model_path='model_output', end=""):
    """保存模型"""
    model_dir = os.path.join(model_path, 'Model_Compare', 'EfficientNetB0')
    os.makedirs(model_dir, exist_ok=True)
    loss_str = f"{loss:.4f}"
    model_name = f'EfficientnetB0-{timestamp}-{epoch}-{loss_str}{end}.pth'
...
def plot_history(train_losses, val_losses, train_accs, val_accs, timestamp):
    ...
    plt.tight_layout()
    model_dir = os.path.join('model_output', 'Model_Compare', 'EfficientNetB0')
    os.makedirs(model_dir, exist_ok=True)
    plt.savefig(os.path.join(model_dir, f'EfficientnetB0-{timestamp}.png'))
    # plt.show()
...
def train_cnn_vit_meta(data_lists, num_epochs, batch_size, learning_rate, resume_training, model_path):
    ...
    if resume_training and model_path:
        ...
        # 兼容旧checkpoint：EfficientNet classifier参数不兼容
        try:
            ...
            for k, v in state.items():
                kl = k.lower()
                if kl.startswith('classifier') or '.classifier.' in kl:
                    dropped.append(k)
                    continue
                filtered_state[k] = v
            load_ret = model.load_state_dict(filtered_state, strict=False)
            print(f"已使用strict=False加载。丢弃参数数: {len(dropped)}; missing={len(getattr(load_ret,'missing_keys',[]))}; unexpected={len(getattr(load_ret,'unexpected_keys',[]))}")
        except Exception as e:
            print(f"兼容加载失败，尝试直接加载: {e}")
    ...
    # 保存报告
    # report_dir = os.path.join('report', 'Model_Compare', 'EfficientNetB0')
    # os.makedirs(report_dir, exist_ok=True)
    # report_path = os.path.join(report_dir, f'EfficientnetB0_report-{timestamp}.txt')
    ...
    try:
        ...
        features, labels = extract_features_for_tsne(
            ...
            needs_3_channels=True  # 预训练模型需要3通道
        )
...
def select_model():
    ...
    model_files = sorted([f for f in os.listdir(model_dir) if f.endswith('.pth') and f.startswith('EfficientnetB0')])
    if not model_files:
